...
const EditSuppModal = ({ isOpen, onClose, supplier, handleChange, handleFileChange, handleSubmit }) => {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: acceptedFiles => {
      ...
      setImageModified(true); // Set imageModified to true when image is changed
    }
  });
            ...
            <form onSubmit={handleSubmit}>
              {/* <div className="form-group mb-3">
                ...
                <div {...getRootProps({ className: 'dropzone' })} >
                  <input {...getInputProps()}/>
                  <p><label for='file' id='uploadbtn'><i className='fas fa-camera'></i></label></p>
                </div>
                {typeof supplier.image === 'string' && ( // Check if supplier.image is a string
                  <img src={`http://localhost:8081/${supplier.image.replace(/\\/g, '/')}`} alt="Supplier Image" style={{ maxWidth: '100%', maxHeight: '200px' }} />
                )}
                {typeof supplier.image !== 'string' && supplier.image && ( // Handle cases where image is not a string
                  <img src={URL.createObjectURL(supplier.image)} alt="Supplier Image" style={{ maxWidth: '100%', maxHeight: '200px' }} />
                )}
              </div> */}
