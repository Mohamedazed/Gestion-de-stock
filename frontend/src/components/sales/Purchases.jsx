...
export default function Purchases() {
  ...
  //       name: searchTerm
  //     };
  //       const response = await axios.get('http://localhost:8081/purchases', { params: queryParams })
  ...
  const fetchData = async () => {
    try {
      ...
      // Sort the products based on the purchase date in descending order
      products.sort((a, b) => new Date(b.Date_Ajout) - new Date(a.Date_Ajout));
    ...
    const Count = () => {
      axios.get('http://localhost:8081/purch_count')
      .then(result => {
  ...
  const handleSearch = async () => {
    setCurrentPage(1);
    try {
      const response = await axios.get(`http://localhost:8081/purchases/search`, {
        params: {
          name: searchTerm,
        }
      });
